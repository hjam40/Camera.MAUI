...
namespace Camera.MAUI.Barcode.ZXing
{
    internal static class Extensions
    {
        ...
        internal static BarcodeResult ToNative(this Result result)
        {
            return new BarcodeResult(result.Text, result.RawBytes, result.ResultPoints.Select(x => new Point(x.X, x.Y)).ToArray(), result.BarcodeFormat.ToNative(), result.ResultMetadata.ToDictionary(k => k.Key.ToString(), v => v.Value), result.NumBits, result.Timestamp);
        }
    }
}